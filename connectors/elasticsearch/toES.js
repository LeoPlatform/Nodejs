		...
		stream: function (settings) {
			...
			let format = ls.through({
				...
				if (deleteByQuery.length) {
					this.getIds(deleteByQuery, client, (err, ids) => {
						if (err) {
							done(err);
						} else {
		...
		getIds: function (queries, client, done) {
			...
			async.eachSeries(queries, (data, callback) => {
				this.query({
					...
					allIds.concat(ids.map(id => id._id));
					callback();
				});
		...
		query: function (data, settings, callback) {
			...
			if (settings.client) {
				...
				let esSettings = extend(true, {
					connectionClass: require('http-aws-es'),
					amazonES: {
						region: leoConfig._meta.region,
						credentials: config.credentials
					}
				}, leoConfig.elasticsearch, settings);
			...
			if (data.scrollid) {
				console.log("Starting As Scroll");
				client.scroll({
					scrollId: data.scrollid,
					scroll: scroll
				}, getUntilDone);
			} else {
				console.log("Starting As Query")
