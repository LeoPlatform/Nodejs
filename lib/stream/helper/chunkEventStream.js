...
module.exports = function (ls, event, opts) {
	...
	var eventStream = ls.buffer({
		...
		if (record.s3) {
			...
			if (s > opts.size * 3 && !opts.disable_too_large_check) {
				let gzipSize = s;
				if (opts.gzip) {
					logger.warn(`Event may be too large for assumed compression of 3, Checking actual compression`);
					gzipSize = Buffer.byteLength(zlib.gzipSync(d));

				}
				logger.log(
					`leo_chunk_stream.event_too_large Event origional: ${s}, compressed: ${gzipSize}, assumed compressed: ${s / 3}, actual compression rate: ${s / gzip_size}`
				);
