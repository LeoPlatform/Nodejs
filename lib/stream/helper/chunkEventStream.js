	...
	var eventStream = ls.buffer({
		...
		function updateCorrelation(c) {
			if (c) {
				...
				if (c.eventOrderIndex != null) {
					correlation.eventOrderIndex = c.eventOrderIndex;
					if (correlation.eventOrderIndexFirst)
						correlation.eventOrderIndexFirst = c.eventOrderIndex;
				}
				delete c.eventOrderIndex;
			}
		}
