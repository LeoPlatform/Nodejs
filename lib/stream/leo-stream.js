		...
		fromLeo: (ID, queue, opts) => {
			...
			dynamodb.docClient.batchGet({
				...
				if (leoEvent.v >= 2 || !leoEvent.max_eid) {
					...
					getEvents = function(start, callback) {
						var params = {
				...
				let getEventsInner = getEvents;
				let getEventsBuffered = [];
				let getEventsStart = start;

				function getNextEvents() {
					getEventsBuffered.push(new Promise((resolve, reject) => {
						console.log(`GET_NEXT_EVENTS start: ${getEventsStart}`);
						getEventsInner(getEventsStart, (err, data) => {
							if (err) {
								reject(err);
							} else {
								if (data.length) {
									getEventsStart = data[data.length - 1].end + "";
								}
								resolve(data);
							}
						});
				...
				getEvents = function(callback) {
					//return getEventsInner(start, callback);
					console.log(`GET_EVENTS start: ${start}, len: ${getEventsBuffered.length}`);
					if (getEventsBuffered.length) {
						let current = getEventsBuffered.shift();
						current.then(data => {
							if (data.length > 0) {
								getNextEvents();
							}
