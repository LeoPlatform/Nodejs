	...
	var ls = {
				...
				if (!correlations && update.correlation_id && update.correlation_id.source) {
					...
					correlations = [{
						[update.correlation_id.source]: {
							...
							end: update.correlation_id.cp_eid || update.correlation_id.end || update.correlation_id.start,
		...
		fromLeo: (ID, queue, opts) => {
			...
			let filterData = {
				...
				read_filter_groups: [],
				maxeid: null,
				...global.rstreamsLeoReadFilterData
			};
			let isFiltered = filterData.icount > 1;
			if (isFiltered) {

				opts.maxOverride = min(opts.maxOverride, filterData.maxeid);
				let filterLogger = logger.sub("filter-" + filterData.iid);
				filterLogger.log(`Reading Queue Filtered. Bot: ${ID}, IID: ${filterData.iid}, ICount: ${filterData.icount}, Queue: ${queue}, Max: ${opts.maxOverride}`);
				...
				// };
				let igroupIndex = 0;
				let read_filter_groups = filterData.read_filter_groups || [];
				const eventPartition = global.rstreamsLeoReadFilterEventPartition;
				pass = ls.pipeline(pass, ls.through((obj, done) => {

					// Find current instance group
					for (; igroupIndex + 1 < read_filter_groups.length && obj.eid >= read_filter_groups[igroupIndex + 1].eid; igroupIndex++) {
						filterLogger.debug(`Past Filter Group, Index: ${igroupIndex}, Range: ${read_filter_groups[igroupIndex].eid} to ${read_filter_groups[igroupIndex + 1].eid}`);
					}


					let icount = (read_filter_groups[igroupIndex] || {}).icount || filterData.icount;
					let partitionValue = eventPartition(obj);
					let partition = Math.abs(hashCode(partitionValue != null ? partitionValue : obj.eid)) % icount;
					filterLogger.debug("[partition]", partition, "[iid]", filterData.iid, "[eid]", obj.eid, "[icount]", icount);
					if (partition == filterData.iid) {
						filterLogger.debug(`------ PROCESSING: matched on partition ------ ${obj.eid}`);
						let toEmit = lastEvent;
						lastEvent = obj;
						if (checkpointMap.eid) {
							if (checkpointMap.eid !== checkpointMap.cpEid) {
								cron.checkpointOverrides.push(checkpointMap);
								toEmit.cp_eid = checkpointMap.cpEid;
							}
							checkpointMap = {
								units: 0
							};
						}
						checkpointMap.eid = obj.eid;
						checkpointMap.units++;
						checkpointMap.cpEid = obj.eid;

						done(null, toEmit);
						//lastEvent = obj;
						//lastEvent.cp_eid = obj.eid;
						// todo: need to fix correlation_id to use eid but pass cp_eid
						//done(null, toEmit.cp_eid ? toEmit : undefined);
					} else {
						filterLogger.debug(`------ NOT PROCESSING: no match on partition ------ ${obj.eid}`);
						checkpointMap.units++;
						checkpointMap.cpEid = obj.eid;
						//lastEvent.cp_eid = obj.eid;
						done();
					...
					let last = lastEvent;
					if (checkpointMap.eid && checkpointMap.eid !== checkpointMap.cpEid) {
						cron.checkpointOverrides.push(checkpointMap);
						last.cp_eid = checkpointMap.cpEid;
					}
					filterLogger.log(JSON.stringify(cron.checkpointOverrides, null, 2));
			...
			dynamodb.docClient.batchGet({
				...
				} else if (leoCron) { //There are no cron jobs, not possible to stream
					if (leoCron.checkpoint && !leoEvent.v) {
						start = leoCron.checkpoint;
					} else if (filterData.iid && leoCron.instances && leoCron.instances[filterData.iid] && leoCron.instances[filterData.iid][queueRef] && leoCron.instances[filterData.iid][queueRef].checkpoint) {
						start = leoCron.instances[filterData.iid][queueRef].checkpoint;
				...
				console.log("Reading event from", start, "Queue:", queueRef, "Bot:", ID, "IID: ", filterData.iid || 0);
		...
		createCorrelation: function(startEvent, endEvent, units, opts) {
			...
			return {
				...
				units: units,
				cp_eid: (endEvent || startEvent).cp_eid
			};
		}
	};
