...
module.exports = function(configure) {
	...
	var ls = {
		...
		stats: (id, event, opts) => {
			...
			let stream = ls.through((event, done) => {
				...
				stats.source_timestamp = Math.min(start, stats.source_timestamp);
				stats.started_timestamp = Math.max(timestamp, stats.started_timestamp);
				stats.ended_timestamp = Math.max(timestamp, stats.ended_timestamp);
				stats.eid = event.eid;
					...
					getEvents = function(callback) {
						var params = {
							...
							ExpressionAttributeValues: {
								":event": queue,
								":maxkey": usingSnapshot ? snapshotEnd.replace("_snapshot/", "") : max_eid,
										...
										if (item.s3) {
											if (item.end.localeCompare(start) > 0) { //Then it is greater than the event they were looking at
												...
												pump(s3Stream, gzipStream, split((value) => {
													...
													let isGreaterThanStart;
													if (usingSnapshot) {
														isGreaterThanStart = e.eid.replace("_snapshot/", '').localeCompare(start) > 0;
													} else {
														isGreaterThanStart = e.eid.localeCompare(start) > 0;
													}



													if (isGreaterThanStart && totalCount < opts.limit && totalSize < opts.size) { //Then it is greater than the event they were looking at
														totalCount++;
														pass.throttledWrite(e, done);
													} else { //otherwise it means we had a number in the middle of a file
														if (totalCount >= opts.limit || totalSize >= opts.size) {
															//we might as well close this this tream;
															s3Stream.destroy();
															gzipStream.destroy();
														}
	...
	if (module.exports.visit) {
		module.exports.visit(ls);
	}
	return ls;
};
