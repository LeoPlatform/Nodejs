...
const s3LocalFileHelper = require("./s3_local_file_helper");
...
module.exports = function(configure) {
	...
	let tmpS3httpOptions;
	if (tmpS3Config.httpOptions) {
		tmpS3httpOptions = tmpS3Config.httpOptions;
		delete tmpS3Config.httpOptions;
	}
	var s3 = new AWS.S3({
	...
	var ls = {
		...
		toLeo: (ID, opts) => {
			let streamId = ls.toLeoStreamId = (ls.toLeoStreamId || 1) + 1;
			opts = opts || {};
			let defaults = {
				s3: {
					records: 1,
			...
			retry.on('ready', function(number, delay) {
				if (records.length === 0) {
					...
					logger.time("firehose request-" + streamId);
					firehose.putRecordBatch({
						...
						if (err) {
							...
							logger.timeEnd("firehose request-" + streamId);
							retry.success();
						}
					});
					...
					logger.debug("sending", records.length, number, delay);
					logger.time("kinesis request-" + streamId);
					let partitionKey = "0";
					...
					kinesis.putRecords({
						...
						if (err) {
							...
							logger.timeEnd("kinesis request-" + streamId);
							retry.success();
						}
					});
				}
		...
		fromLeo: (ID, queue, opts) => {
			...
			dynamodb.docClient.batchGet({
				...
				s3LocalFileHelper.verifyLocalDirectory();
				...
				if (usingSnapshot || max_eid.localeCompare(start) > 0) {
					async.whilst(() => {
						...
						getEvents(function(err, items) {
							...
							let downloadQueue = async.queue(function(task, callback) {
								...
								require("end-of-stream")(stream, (err) => {
									if (err) {
										...
										try {
											loggerS3.debug(`Error downloading s3: ${bucket}/${key} file: ${localFilename}`);
											fs.unlinkSync(localFilename);
										} catch (err) {
											loggerS3.error(`Error deleting local file: ${localFilename}`);
										}
									} else {
										loggerS3.debug(`Complete downloading s3: ${bucket}/${key} file: ${localFilename}`);
									}

									loggerS3.debug(`ending task-normal for: ${bucket}/${key}`);
							...
							let close_remaining_s3_streams = (cb) => {
								downloadQueueExiting = true;
								loggerS3.debug("closing all remaining s3 streams", Object.keys(s3_streams).length);

								Object.entries(s3_streams).forEach(([key, data]) => {

									loggerS3.debug("attempting close s3 stream:", key, !!data.stream);
									if (data.stream) {
										data.stream.on('error', (err) => {
											// Ignore errors.  This stream was never used
											loggerS3.debug("Error closing s3 stream:", key, err);
										});
										loggerS3.debug("closing s3 stream:", key);
										data.stream.destroy();
									}
								});
								...
								if (downloadQueue.idle()) {
									...
									loggerS3.debug("waiting for queue to drain", downloadQueue.workersList());
									downloadQueue.drain = () => {
										loggerS3.debug("waiting for queue to drain done", downloadQueue.length());
										cb();
									};
								}
							};
							...
							let get_s3_stream = (index) => {
								...
								for (; last_s3_index <= index || (last_s3_index < items.length && bytes < opts.fast_s3_read_parallel_fetch_max_bytes); last_s3_index++) {
									...
									if (item.s3 != null && item.gzipSize <= opts.fast_s3_read_max_download_file_size_bytes) {
										...
										bytes += myBytes;
										loggerS3.debug(last_s3_index, "setting up bytes", myBytes, bytes, item.s3.key);
										s3_streams[last_s3_index] = {
											bytes: myBytes,
											stream: create_s3_stream(item)
										};
								...
								if (items[index].s3.localFileReady) {
									...
									items[index].s3.localFileReady.then(() => {
										ls.pipe(
											...
											(err) => {
												...
												if (!err) {
													try {
														loggerS3.debug(`deleting local s3 file: ${items[index].s3.localFilename}`);
														if (!opts.fast_s3_read_save_files) {
															fs.unlinkSync(items[index].s3.localFilename);
														}
													} catch (err) {
														loggerS3.error(`error deleting local s3 file: ${items[index].s3.localFilename}`);
													}
												}
											});
									...
									return ls.fromS3(items[index].s3);
								}
							};
							...
							let free_s3_stream = (index) => {
								if (s3_streams[index] != null) {
									...
									loggerS3.debug("free s3 connection", s3_streams[index].bytes, bytes, items[index].s3.key);
								}
							...
							let create_s3_stream = (item) => {
								if (item.s3 && item.end.localeCompare(start) > 0) {
									...
									let theStream;
									let destroy = (...args) => {
										loggerS3.debug(`Destroying s3: ${bucket}/${key} file: ${file.localFilename}`);
										theStream && theStream.destroy(...args);
									};
									let on = (...args) => {
										theStream && theStream.on(...args);
									};
									let setStream = function(stream) {
										theStream = stream;
									};
									loggerS3.debug(`pushing to queue start: ${bucket}/${key}`);

									downloadQueue.push({ file: file, setStream }, (err) => {
										loggerS3.debug(`pushing to queue end: ${bucket}/${key}`, err || "");
										err ? rej(err) : res();
									});
									return { destroy, on };
								}
								...
								if (item.s3) {
									...
									if (item.end.localeCompare(start) > 0) { //Then it is greater than the event they were looking at
										...
										pump(
											...[
												s3Stream,
												s3Stream.uncompressed ? undefined : gzipStream,
												split((value) => {
													try {
														return {
