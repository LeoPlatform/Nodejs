...
module.exports = function(configure) {
	...
	[awsResourceConfig.s3Config, awsResourceConfig.kinesisConfig, awsResourceConfig.dynamodbConfig, awsResourceConfig.firehoseConfig].forEach(config => {
		if (config != null) {
	...
	let sdkId = "-" + (Math.random() * 10000).toString();
	let sdkTimeIndex = 1;
	...
	var ls = {
		...
		fromLeo: (ID, queue, opts) => {
			...
			const JSONparse = parserUtil.createParser({
				parser: opts.parser,
				opts: {
					...opts.parserOpts
				}
							...
							async.eachOfSeries(items, (item, i, done) => {
								...
								if (item.end.localeCompare(start) > 0) {
									if (item.stream) {
										...
										readStream = pump(s3Stream, gzipStream, splitParseStream);
									} else if (item.gzip) {
										...
										readStream = pump(gzip, splitParseStream);
									}
								}
									...
									pump(
										...
										ls.write((e, sinkDone) => {
											...
											if (!isPassDestroyed && isGreaterThanStart && totalCount < opts.limit && totalSize < opts.size) { //Then it is greater than the event they were looking at
												...
												logger.debug("skipping generic stream", start, e.eid);
												sinkDone();
											}
										}), (err) => {
											if (err &&
