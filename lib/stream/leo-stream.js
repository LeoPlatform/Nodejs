...
const { promiseStreams } = require('leo-streams');
...
module.exports = function(configure) {
	...
	let cron = cronlib(configure);
	let pad = "0000000";
	let padLength = -1 * pad.length;
	let ls = {
		...
		tryFinalizeMultipartUpload: async (file) => {
			...
			if (mpResponse.Uploads.length) {
				...
				let parts = allParts.Parts.map((p) => ({ ETag: p.ETag,
		...
		enrich: function(opts, callback) {
			configure.validate();
			let id = opts.id;
			let inQueue = opts.inQueue;
			let outQueue = opts.outQueue;
			let func = opts.transform || opts.each;
		...
		offload: function(opts, callback) {
			configure.validate();
			let id = opts.id;
			let inQueue = opts.inQueue || opts.queue;
			let func = opts.each || opts.transform;
			let batch = {
				size: 1,
			...
			let batchSize = typeof batch.size === "number" ? batch.size : (batch.size.count || batch.size.records);
			return ls.pipe(
				ls.fromLeo(id, inQueue, opts),
				ls.through((obj, done) => {
					batch.map(obj.payload, obj, (err, r) => {
						if (err || !r) {
							done(err);
						} else {
							obj.payload = r;
		...
		toLeo: (ID, opts) => {
					...
					firehose.putRecordBatch({
						...
						if (err) {
							...
							for (let i = 0; i < data.RequestResponses.length; i++) {
								let row = data.RequestResponses[i];
								if (row.ErrorCode) {
									left.push(records[i]);
								}
							}
			...
			p = ls.buffer(opts, function(obj, callback) {
				if (obj.records > 0) {
					if (obj.gzip) {
			...
			let streams = [];
			if (!opts.useS3 && !opts.firehose) {
				streams.push(promiseStreams.throughPromise(async (obj, push) => {
					let size = Buffer.byteLength(JSON.stringify(obj));
					if (size > twoHundredK * 3) {
						logger.info('Sending event to S3 because it exceeds the max size for DDB. ', size);
						await promiseStreams.pipelineAsync(
		...
		checkpoint: (opts, stream) => {
			...
			stream.on("checkpoint", (opts) => {
				stream.checkpoint(opts || {}, () => { /* Do Nothing */ });
			});
		...
		toCheckpoint: (opts) => {
			...
			function doCheckpoint(callback) {
				...
				for (let id in checkpoints) {
					let bot = checkpoints[id];
					for (let event in bot) {
						let checkpoint = bot[event];
						tasks.push((done) => {
			...
			let result = ls.buffer({
				...
				let c = checkpoints[id];

				let records = 0;
				correlations.forEach((correlation) => {
					for (let event in correlation) {
						let u = correlation[event];
						if (!(event in c)) {
							c[event] = {
								eid: u.end,
								records: u.records,
		...
		fromLeo: (ID, queue, opts) => {
			...
			if (!opts.stopTime && opts.runTime) {
				opts.stopTime = moment().add(opts.runTime)
					.valueOf();
			}
			...
			pass.destroy = function() {
				...
				if (hasTimeTimeout != null) {
					clearTimeout(hasTimeTimeout); 
				}
			};
			...
			dynamodb.docClient.batchGet({
				...
				let queueRef = refUtil.refId(queue);
				...
				if (leoEvent.v >= 2 || !leoEvent.max_eid) {
					max_eid = (configure.registry && configure.registry.__cron && configure.registry.__cron.maxeid) || opts.maxOverride || leoEvent.max_eid || '';
					table_name = configure.resources.LeoStream;
					range_key = "end";
					...
					max_eid = max(leoEvent.kinesis_number, leoEvent.s3_kinesis_number, leoEvent.initial_kinesis_number, leoEvent.s3_new_kinesis_number);
					table_name = "Leo";
		...
		putEvent: function(id, event, obj, opts, callback) {
			...
			let stream = ls.load(id, event, opts);
			stream.write(obj);
			stream.end((err) => {
				if (err) {
