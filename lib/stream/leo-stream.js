...
module.exports = function(configure) {
	...
	var ls = {
		...
		fromLeo: (ID, queue, opts) => {
			...
			const JSONparse = opts.parser || JSON.parse;
							...
							async.eachOfSeries(items, (item, i, done) => {
								...
								if (item.s3) {
									...
									if (item.gzip && leoEvent.v >= 2) {
										try {
											item.payload = JSONparse(zlib.gunzipSync(item.payload).toString());
										} catch (e) {
											item.payload = {};
										}
									} else if (typeof item.payload === "string") {
										item.payload = JSONparse(item.payload);
									}
									...
									pump(gzip, split(JSONparse), ls.write((e, done) => {
