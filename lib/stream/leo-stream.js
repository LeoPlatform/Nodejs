...
module.exports = function (configure = {}) {
	...
	var ls = {
		...
		enrich: function (id, inQueue, func, outQueue, callback) {
			return ls.pipe(ls.fromLeo(id, inQueue), ls.process(id, func, outQueue), ls.toLeo(id), ls.toCheckpoint(), callback);
		},
		offload: function (id, inQueue, func, callback) {
			return ls.pipe(ls.fromLeo(id, inQueue), ls.process(id, func, null));
		},
		...
		toLeo: (ID, opts = {}) => {
			...
			retry.on('ready', function (number, delay) {
				if (records.length === 0) {
					...
					kinesis.putRecords({
						...
						StreamName: 'Leo-KinesisStream-ATNV3XQO0YHV'
					}, function (err, data) {
						if (err) {
							retry.backoff();
			...
			return ls.pipeline(chunkEventStream(ls, {
				records: 10000,
				size: 1024 * 300,
				time: {
					seconds: 10
