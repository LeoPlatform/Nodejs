...
module.exports = function (configure = {}) {
	...
	var ls = {
		...
		logSourceRead: function (id, inSystem, recordCount, opts, callback) {
			...
			opts = opts || {};
			if (!inSystem.match(/\./)) {
				inSystem = "system." + inSystem;
			}
	
			this.checkpoint(id, inSystem, {
		...
		logTargetWrite: function (id, outSystem, recordCount, opts, callback) {
			...
			outSystem = refUtil.ref(outSystem, "system").asQueue().id;
		...
		load: function (id, outQueue, opts) {
			...
			}), ls.toLeo(id, opts), ls.toCheckpoint());
		},
		...
		offload: function (opts, callback) {
			...
			var batch = {
				size: 1,
				map:  (payload, meta, done) => done(meta)
			};
		...
		fromLeo: (ID, queue, opts = {}) => {
			...
			if (!opts.stopTime && opts.runTime){
				opts.stopTime = moment.now().add(opts.runTime).valueOf()
			}
			opts = Object.assign({
				buffer: 1000,
				loops: 100,
			...
			dynamodb.docClient.batchGet({
				...
				if (opts.start) {
					...
					if (leoCron.checkpoint && !leoEvent.v) {
						...
						start = leoCron.checkpoints.read[queueRef].checkpoint;
					} else {
						start = "z/";
					}
				} else {
				...
				if (leoEvent.v >= 2) {
					...
					getEvents = function (callback) {
						leo.getEvents(ID, queueRef, Object.assign({}, opts, {
							start: start
