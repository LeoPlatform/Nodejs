			...
			dynamodb.docClient.batchGet({
				...
				if (leoEvent.v >= 2 || !leoEvent.max_eid) {
					...
					getEvents = function(callback) {
						...
						if (params.ExpressionAttributeValues[":start"] >= params.ExpressionAttributeValues[":maxkey"]) {
							callback(null, []);
							return;
						}
						logger.debug(params);
				...
				let pollSettings = {
					intervalMS: function(count, waitTimeMS) {
						return waitTimeMS < 10000 ? 200 : 1000;
					},
					maxWaitMS: 2_000,
					...(typeof opts.poll !== "boolean") ? opts.poll : undefined,
					enabled: !!opts.poll,

					consecutiveEmpty: 0,
					noEventStart: null,
					getCurrentWaitMS: function() {
						let now = Date.now();
						return now - (this.noEventStart || now);
					},
					test: function() {
						return !this.enabled || this.getCurrentWaitMS() < this.maxWaitMS;
					},
					preGetEvents: function() {
						if (this.enabled) {
							max_eid = ls.eventIdFromTimestamp(Date.now());
						}
					},
					onNoEvents: function() {
						if (this.noEventStart == null) {
							this.noEventStart = Date.now();
						}
					...
					reset: function() {
						this.noEventStart = null;
						this.consecutiveEmpty = 0;
					}
				};

				if (usingSnapshot || max_eid.localeCompare(start) > 0 || pollSettings.enabled) {
					async.whilst(() => {
						...
						logger.debug(totalCount, opts.limit, totalSize, opts.size, hasMoreEvents, hasTime, hasLoops, max_eid, start, (usingSnapshot || max_eid.localeCompare(start)), pollSettings.getCurrentWaitMS());
						return totalCount < opts.limit && totalSize < opts.size && hasMoreEvents && hasTime && hasLoops && (usingSnapshot || max_eid.localeCompare(start) > 0 || pollSettings.test());
					}, (done) => {
						var count = 0;
						hasLoops--;
						pollSettings.preGetEvents();
						getEvents(function(err, items) {
							...
							if (items.length == 0) {
								if (usingSnapshot) { //Then let's go back to the real queue
									...
									if (pollSettings.onNoEvents()) {
										let currentWait = pollSettings.getCurrentWaitMS();
										let waitMS = pollSettings.intervalMS(pollSettings.consecutiveEmpty, currentWait);
										logger.debug(`Waiting for ${waitMS} ms, ${currentWait}`);
