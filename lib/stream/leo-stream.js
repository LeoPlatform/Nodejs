		...
		fromLeo: (ID, queue, opts) => {
			...
			dynamodb.docClient.batchGet({
				...
				if (docs.Responses && docs.Responses[EVENT_TABLE] &&
					...
					if (docs.Responses[EVENT_TABLE][0].snapshot) {
						snapshotNext = docs.Responses[EVENT_TABLE][0].snapshot.next;
						snapshotStart = docs.Responses[EVENT_TABLE][0].snapshot.start;
					}
					if (docs.Responses[EVENT_TABLE][0].archive) {
						archive_end = docs.Responses[EVENT_TABLE][0].archive.end;
					}
				}
