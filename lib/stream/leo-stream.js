	...
	var ls = {
		...
		fromLeo: (ID, queue, opts) => {
			...
			dynamodb.docClient.batchGet({
				...
				if (docs.Responses && docs.Responses[EVENT_TABLE] &&
					docs.Responses[EVENT_TABLE][0]) {
					if (docs.Responses[EVENT_TABLE][0].snapshot_start) {
						snapshotNext = docs.Responses[EVENT_TABLE][0].snapshot_next;
						snapshotStart = docs.Responses[EVENT_TABLE][0].snapshot_start;
					}
					if (docs.Responses[EVENT_TABLE][0].archive_end) {
						archive_end = docs.Responses[EVENT_TABLE][0].archive_end;
					}
				}
