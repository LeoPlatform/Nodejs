...
module.exports = function (configure) {
	...
	configure.onUpdate((newConfigure) => {
		...
		[kinesis, firehose, s3].map(service => {
			let serviceName = service.endpoint.hostname.split(".")[0];
			service.endpoint = new AWS.Endpoint(`${serviceName}.${newConfigure.aws.region}.amazonaws.com`)
		...
		toLeo: (ID, opts) => {
			...
			retry.on('ready', function (number, delay) {
				if (records.length === 0) {
					...
					let remaining = [];
					records.forEach(r => {
						let rec_size = Buffer.byteLength(r);
						if (to_send_size === 0 || to_send_size + rec_size < opts.size) {
							to_send_size += rec_size;
							to_send += r;
						} else {
							remaining.push(r);
						}
					...
					firehose.putRecordBatch({
						...
						if (err) {
							...
							records = left.concat(remaining);
							retry.backoff();
						} else if (remaining.length > 0) {
							// Was successful so add the remaining and reset the backoff
