...
module.exports = function(configure) {
	...
	var ls = {
		...
		fromLeo: (ID, queue, opts) => {
			if (global.fanoutOverrides && global.fanoutOverrides.fromLeo) {
				return global.fanoutOverrides.fromLeo.call(ls, ID, queue, opts);
			}
			opts = opts || {};
			...
			dynamodb.docClient.batchGet({
				...
				if (leoEvent.v >= 2 || !leoEvent.max_eid) {
					...
					getEvents = function(callback) {
						...
						logger.debug(params);
						let queryDBFn = (global.fanoutOverrides && global.fanoutOverrides.fromLeoDbQueryFn && global.fanoutOverrides.fromLeoDbQueryFn) || dynamodb.docClient.query.bind(dynamodb.docClient);

						queryDBFn(params, function(err, data) {
							...
							async.eachOfSeries(items, (item, i, done) => {
								...
								if (item.s3) {
									...
									if (item.gzip && leoEvent.v >= 2) {
										try {
											item.payload = opts.parse(zlib.gunzipSync(item.payload).toString());
										} catch (e) {
											item.payload = {};
										}
									} else if (typeof item.payload === "string") {
										item.payload = opts.parse(item.payload);
									}
									...
									pump(gzip, split((data) => {
										let r = opts.parse(data);
										if (r.size == null) {
											r.size = Buffer.byteLength(data);
										}
										return r;
									}), ls.write((e, done) => {
