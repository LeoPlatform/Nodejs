	...
	var ls = {
		...
		toLeo: (ID, opts) => {
			...
			retry.on('ready', function (number, delay) {
				if (records.length === 0) {
					...
					opts.debug && console.time("firehose request");
					firehose.putRecordBatch({
						...
						opts.debug && console.log(process.memoryUsage());
						if (err) {
							console.log(err);
							retry.backoff();
							...
							opts.debug && console.timeEnd("firehose request");
							retry.success();
						}
					});
			...
			dynamodb.docClient.batchGet({
				...
				if (max_eid.localeCompare(start) > 0) {
					async.whilst(() => {
							...
							opts.debug && console.log(totalCount, opts.limit, totalSize, opts.size, hasMoreEvents, hasTime, hasLoops, max_eid, start, max_eid.localeCompare(start));
							...
							getEvents(function (err, items) {
								...
								async.eachOfSeries(items, (item, i, done) => {
										...
										function toEId(eid) {
											return prefix + "-" + (pad + eid).slice(padLength);
										};
										if (item.s3) {
											if (item.end.localeCompare(start) > 0) { //Then it is greater than the event they were looking at
												...
												for (let i = 0; i < item.offsets.length; i++) {
													...
													if (start.localeCompare(toEId(offset.end)) < 0) {
												...
												pump(ls.fromS3(file), zlib.createGunzip(), split((value) => {
													...
													if (e.eid.localeCompare(start) > 0 && totalCount < opts.limit && totalSize < opts.size) { //Then it is greater than the event they were looking at
														...
														opts.debug && console.log("skipping s3", start, e.eid);
														done();
													}
												}), (err) => {
													if (err) {
											...
											if (item.eid.localeCompare(start) > 0 && totalCount < opts.limit && totalSize < opts.size) { //Then it is greater than the event they were looking at
												...
												opts.debug && console.log("skipping gzip");
												done();
											}
										} else if (item.gzip) {
											...
											pump(gzip, split(JSON.parse), ls.write((e, enc, done) => {
												...
												if (e.eid.localeCompare(start) > 0 && totalCount < opts.limit && totalSize < opts.size) { //Then it is greater than the event they were looking at
													...
													opts.debug && console.log("skipping gzipped");
													done();
												}
											}), (err) => {
												if (err) {
		...
		toDynamoDB: function (table, opts) {
			...
			return ls.buffer({
				...
				console.log("toDynamoDB On Flush");
				done();
			});
		},
