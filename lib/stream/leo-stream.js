...
module.exports = function(configure) {
	...
	const s3Timeout = process.env.S3_TIMEOUT_MS ? parseInt(process.env.S3_TIMEOUT_MS) : 120000;
	...
	var ls = {
					...
					async.whilst(() => {
						...
						getEvents(function(err, items) {
							...
							let get_s3_stream = (index) => {
								...
								if (!s3_streams[index]) {
									/**
									 * This function is called as part of an async eachOfSeries call, which means
									 * that only one async call is ever running at a time. Therefore, if index is >= last_s3_index
									 * and we have NOT created a stream, then we need to, as we are now on the section where we 
									 * will be reading the data from the S3 file, so the stream MUST exist for index when returning from
									 * this function. We will therefore use nearly the same logic as above to create a new stream, and we
									 * will also set last_s3_index to index + 1 so we don't overwrite or re-create the stream the next time
									 * getStream is called.
									 */
									console.log(`index ${index} is >= last_s3_index ${last_s3_index}, and an S3 stream does not exist. Creating one now`);
									if (items[index].s3 != null) {
										// Add to the total bytes waiting to be processed
										bytes += agg_bytes;
										s3_streams[index] = {
											bytes: agg_bytes,
											stream: create_s3_stream(items[index])
										};
										agg_bytes = 0;
										last_s3_index = index + 1;
									}
								}
		...
		buffer2: _streams.buffer2,
		bufferBackoff: _streams.bufferBackoff,
		batch: _streams.batch,
		batchFilter: _streams.batchFilter,
