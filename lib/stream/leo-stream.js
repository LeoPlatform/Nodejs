...
const { StreamUtilV2 } = require("./leo-stream-v2");
						...
						getEvents(function(err, items) {
							...
							let get_s3_stream = (index) => {
								...
								if (items[index].s3.localFileReady) {
									...
									let s = ls.fromS3(items[index].s3);
							...
							let create_s3_stream = (item, index) => {
								...
								let s3key = `s3: ${item.s3.bucket}/${item.s3.key}`;
								...
								if (item.s3 && item.end.localeCompare(start) > 0) {
									...
									downloadQueue.push({ file: file, setStream }, (err) => {
										...
										err ? localFileReadyReject(err) : localFileReadyResolve();
									});
									return { destroy, on };
								}
								return null;
...
function mergeV2(ls) {
	...
	[StreamUtilV2]
		.concat(global.rstreamsOverrides || [])
		.concat(process.rstreamsOverrides || [])
		//.concat(/* need to use RStreams Global Var here */)
		//.concat(/* need to use RStreams passed in var here */)
		.forEach(fn => {
			let newLs = new fn(ls);
