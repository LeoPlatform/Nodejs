...
function createBotInvocationEvent(botId, settings = {}) {
	return {
		...settings,
		botId: botId,
		__cron: {
...
function overrideLeoFunctions (data = {}, leo = leosdk) {
	leo.mocked = true;
	...
	leo.mock = leo.mock || { createContext,
		getData: () => leo.mock[testId] };
	leo.mock[testId] = {};
	let mock = leo.mock[testId];

	// define various overrides of Leo functions
	if (data.read) {
		leo.streams.fromLeo = (id, queue) => {
	...
	if (data.toDynamoDB) {
		if (isWritableStream(data.toDynamoDB)) {
			...
			leo.streams.toDynamoDB = (tableName) => {
				mock.dynamodb.data[tableName] = mock.dynamodb.data[tableName] || {};
				let table = mock.dynamodb.data[tableName];
