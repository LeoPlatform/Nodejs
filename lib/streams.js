...
let ls = module.exports = {
	...
	buffer: (opts, each, emit, flush) => {
		...
		function doEmit(last = false, done) {
			if (records) {
				opts.debug && console.log(`${label}emitting ${last?'due to stream end':''} ${isSizeConstrained?'due to size':''} ${isTimeConstrained?'due to time':''}  ${isRecordConstrained?'due to record count':''}  records:${records} size:${size}`);
				emitting = true;
				let data = {
					records: records,
					size: size,
				...
				emit.call(stream, (err, obj) => {
					emitting = false;
					opts.debug && console.log(label + "emitting done");
					if (obj != undefined) {
						stream.push(obj)
					}
					if (done) {
					...
					if (buffered) {
						opts.debug && console.log(label + "had a buffered one");
						doEach(buffered.o, buffered.done);
						buffered = null;
					}
				}, data);
			} else {
				opts.debug && console.log(label + "Stream ended");
				done(null, null);
			}
		}
