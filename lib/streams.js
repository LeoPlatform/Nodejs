...
let ls = module.exports = {
	...
	buffer: (opts, each, emit, flush) => {
		...
		function doEmit(last = false, done) {
			if (records) {
				logger.debug(`${label}emitting ${last ? 'due to stream end' : ''} ${isSizeConstrained ? 'due to size' : ''} ${isTimeConstrained ? 'due to time' : ''}  ${isRecordConstrained ? 'due to record count' : ''}  records:${records} size:${size} duration:${Date.now() - (startTime || 0)}`);
				let data = {
					records: records,
					size: size,
					isLast: last
		...
		var stream = streamType(merge({
			...
			each.call(stream, o, (err, obj) => {
				if (obj) {
					...
					records += typeof obj.records == "number" ? obj.records : 1;
					size += typeof obj.size == "number" ? obj.size : 1;
					if (!timeout) {
						timeout = setTimeout(() => {
							isTimeConstrained = true;
							stream.writable && stream.write({
	...
	batchFilter: function(opts) {
		...
		let push = (stream, data) => {
			...
			if (totalUnits > 0) {
				...
				let firstCorrelation = first.correlation_id || {};
				let lastCorrelation = last.correlation_id || {};
				let correlation_id = {
					source: firstCorrelation.source,
					start: firstCorrelation.start,
					end: lastCorrelation.end || lastCorrelation.start,
					units: units
				};
				stream.push({
					id: first.id,
					...
					bytes: data.size,
					correlation_id: correlation_id,
					event_source_timestamp: first.event_source_timestamp,
		...
		let sizeOpts = opts.size || opts.bytes;
		let recordOpts = opts.records || opts.count;
		let filterOpts = opts.filter;
		var stream = ls.buffer2({
			label: "batchFilter",
			time: opts.time,
			size: typeof sizeOpts === "number" ? sizeOpts : Number.POSITIVE_INFINITY,
			records: typeof recordOpts === "number" ? recordOpts : Number.POSITIVE_INFINITY,
			highWaterMark: opts.highWaterMark,
			debug: opts.debug
		}, function(obj, callback) {
			if (firstEvent == null) {
