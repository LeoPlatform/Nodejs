...
module.exports = function (configure) {
	...
	return {
		...
		checkpoint: function (id, event, params, callback) {
			...
			if (type == "read" && !params.force && (params.expected || (configure.registry && configure.registry.__cron))) {
		...
		start: function (event, opts, callback) {
			...
			let get__cron = (d) => {
				...
				this.get(event.id || event.botId, {}, (err, data) => {
					...
					console.log(`${cron.id} - ${cron.name} - ${cron.iid} Adding lock`)
			...
			let doCheckLock = (resolve, reject, callback) => {
				if (event.__cron && opts.lock) {
					...
					this.checkLock(event.__cron, configure.registry.__cron.__requestId, context && context.getRemainingTimeInMillis && context.getRemainingTimeInMillis() || 24 * 60 * 1000, function (lockErr, lockData) {
			...
			return new Promise((resolve, reject) => {
				...
				if (!configure.registry.__cron) {
					console.log("No bot data registered");
					callback();
					resolve();
					return;
				}
		...
		createBot: function (id, bot) {
			return new Promise((resolve, reject) => {
				...
				if (bot.settings.source) {
					bot.triggers.push(bot.settings.source);
				}

				console.log("Updating Cron Entry", id, bot)
				dynamodb.update(CRON_TABLE, {
					...
					console.log(err);
					if (err) {
						console.log("Error Saving Cron", err);
						reject(err);
					} else {
						resolve({});
					}
