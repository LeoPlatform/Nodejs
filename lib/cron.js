...
module.exports = function(configure) {
	configure = configure || {};
	let dynamodb = new Dynamo(configure);
	...
	let SETTINGS_TABLE = configure.resources.LeoSettings;
	let SYSTEM_TABLE = configure.resources.LeoSystem;
	return {
		...
		reportComplete(cron, runid, status, log, opts, callback) {
			...
			if (opts.forceComplete) {
				command = {
					...
					ExpressionAttributeValues: {
						":value": {
							...
							result: cron.result != null ? zlib.gzipSync(JSON.stringify(cron.result)) : null,
				...
				command = {
					...
					ConditionExpression: '#instances.#index.#token = :token',
					ExpressionAttributeNames: {
						"#instances": "instances",
						"#index": (cron.iid || 0).toString(),
		...
		checkpoint: function(id, event, params, callback) {
			...
			let type = params.type === "write" ? "write" : "read";
			...
			if (type === "read" && !params.force && (!configure.registry ||
				...
				let expected = params.expected || (configure.registry &&
			...
			let updateInMemoryCheckpoint = () => {
				// this path is not guaranteed to be in config so do a safe set
				let c = ["registry", "__cron", "checkpoints", type].reduce((o, f) => {
					o[f] = o[f] || {}; 
					return o[f]; 
				}, configure);
				...
				if (err && err.code === "ConditionalCheckFailedException") {
					...
					dynamodb.get(CRON_TABLE, id, {}, function(getErr, getData) {
						...
						if (getErr) {
							...
							dynamodb.put(CRON_TABLE, id, entry, function(err) {
								if (err) {
		...
		getCheckpoint: async function(botId, queue) {
			...
			let checkpointData = ["registry", "__cron", "checkpoints", "read"].reduce((o, f) => {
				o[f] = o[f] || {};
				return o[f];
			}, configure);
		...
		getLastResult: function() {
			let r = configure.registry.__cron && configure.registry.__cron.lastResult;
			return r != null ? JSON.parse(zlib.gunzipSync(Buffer.from(r, "base64"))) : null;
		},
		...
		getAttachedSystemByAlias: function(alias) {
			let systems = (configure.registry && configure.registry.__cron && configure.registry.__cron.systems) || {};
			return Object.keys(systems).map((key) => systems[key])
				.filter((system) => system.aliases.indexOf(alias) > -1)[0];
		},
		getAttachedSystem: function(id) {
			id = refUtil.ref(id).id.replace(/^system\./, "");
			let systems = (configure.registry && configure.registry.__cron && configure.registry.__cron.systems) || {};
			return systems[id];
		},
		get: function(id, opts, callback) {
			...
			dynamodb.get(CRON_TABLE, id, (err, data) => {
				if (err) {
					...
					}).then((r) => callback(null, single ? r[0] : r))
						.catch(callback);
				}
			});
		},
		buildPayloads: function(cron, prev, opts) {
			...
			return new Promise((resolve, reject) => {
				...
				let setting = (cron.lambda && cron.lambda.settings) || {};
				...
				if (setting.destination) {
					...
					if (sref.type === "system") {
						systemIds[sref.id] = systemIds[sref.id] || [];
						systemIds[sref.id].push("destination");
					}
				}
				...
				let applyInstances = function(cronSettings) {
					let results = [];
					let instanceIds = opts.instances || ["0"];
					for (let index in instanceIds) {
						let iPayload = extend(true, {}, cronSettings, setting, (cron.namedSettings || {})[instanceIds[index].toString()]);
				...
				if (cron.system && cron.system.id) {
					...
					systemIds[sref.id] = systemIds[sref.id] || [];
					systemIds[sref.id].push("default");
				}
				...
				if (Object.keys(systemIds).length) {
					dynamodb.docClient.batchGet({
						...
						if (err) {
							...
							Object.keys(systemIds).forEach((id) => {
		...
		shouldRun: function(oldImage, newImage, cache, callback) {
			...
			let delay = (newImage.delay && moment.duration(newImage.delay).asMilliseconds()) || 0;
			let completedTime = 0;
			let isRunning = false;
			let timeout = 0;
			let invokeTime = newImage.invokeTime || 0;
			let instances = cache._instances[newImage.id];
			...
			if (newImage.errorCount > 10 || (newImage.paused && !newImage.ignorePaused) || !hasCode) {
				...
				let results = Object.keys(newImage.namedTrigger).map((key) => {
					let source = (newImage.namedSettings && newImage.namedSettings[key] && newImage.namedSettings[key].source) || (newImage.lambda && newImage.lambda.settings && newImage.lambda.settings[0] && newImage.lambda.settings[0].source) || undefined;
					let moreToDo = this.hasMoreToDo(oldImage, newImage, source);
					...
					return {
						...
						!paused: ${!newImage.paused} || ignorePaused: ${Boolean(newImage.ignorePaused)}
				...
				let now = moment.now();
				let moreToDo = this.hasMoreToDo(oldImage, newImage);
				let result = 
					(!isRunning || invokeTime + timeout < now) &&
					hasCode &&
					(!sameToken || moreToDo) &&
		...
		hasMoreToDo: function(oldImage, newImage, key) {
			let reads = (newImage && newImage.checkpoints && newImage.checkpoints.read) || {};
			let allowed = {};
			if (key) {
				allowed = {
					[key]: true
				...
				newImage.triggers.forEach((q) => {
					allowed[q] = true;
				});
			}
			return (newImage &&
				newImage.requested_kinesis &&
				Object.keys(newImage.requested_kinesis).filter((k) => allowed[k])
					.reduce((result, event) => {
						let latest = newImage.requested_kinesis[event];
						let checkpoint = reads[event];
						return result || !checkpoint || latest > checkpoint.checkpoint;
					}, false)) || false;

		},
		start: function(event, opts, callback) {
			event.__cron = event.__cron || {
				id: event.id || event.botId
			};
			configure.registry.__cron = event.__cron;
			...
			let get__cron = (d) => {
				...
				this.get(event.id || event.botId, {}, (err, data) => {
					...
					event.__cron = data.__cron;
					configure.registry.__cron = data.__cron;
					let cron = data.__cron;
					cron.ts = Date.now();
					let index = cron.iid;
					configure.registry.__cron.__requestId = (opts && opts.requestId) || Math.round(Math.random() * 10000000000000000).toString();

					let newInvokeTime = moment.now();
					let command = {
						TableName: CRON_TABLE,
						Key: {
							id: cron.id
					...
					if (!cron.checkpoints) {
						...
						logger.log("Bot Didn't exist in Cron.  Saving Cron Entry");
						dynamodb.put(CRON_TABLE, cron.id, entry, function(err) {
							d(err);
						});
					} else {
						dynamodb.docClient.update(command, function(err) {
			...
			let hasCallback = Boolean(callback);
			...
			let doCheckLock = (resolve, reject, callback) => {
				if (event.__cron && opts.lock) {
					...
					this.checkLock(event.__cron, configure.registry.__cron.__requestId, opts.timeout || (context && context.getRemainingTimeInMillis && context.getRemainingTimeInMillis()) || 24 * 60 * 1000, function(lockErr) {
