...
var Ref = function(data, opts) {
	...
	this.id = (data.id == null ? data : data.id).toString().replace(/^[seb]_/, "");
...
Ref.prototype.queue = function(subQueue) {
	...
	let regex = new RegExp(`^${this.type}\\.`);
	return new Ref({
...
module.exports = {
	...
	fixBotReferences: function(bot, opts) {
		...
		let settings = (bot.lambda && bot.lambda.settings && bot.lambda.settings[0]) || {};
		...
		if (opts.checkpoints) {
			let checkpoints = {};
			Object.keys(bot.checkpoints).forEach((type) => {
				var obj = {};
				checkpoints[type] = obj;
				Object.keys(bot.checkpoints[type]).forEach((id) => {
	...
	fixSystemReferences: function(system, opts) {
		...
		if (opts.checksums && system.checksums) {
			let checksums = {};
			Object.keys(system.checksums).forEach((id) => {
