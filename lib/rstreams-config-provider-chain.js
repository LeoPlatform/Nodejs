        ...
        if (self.resolveCallbacks.push(callback) === 1) {
            ...
            function resolveNext(err, creds) {
                if ((!err && creds) || index === providers.length) {
                    aws_util_1.default.arrayEach(self.resolveCallbacks, function (callback) {
                        callback(err, creds);
                    });
...
exports.ConfigProviderChain.addPromisesToClass = function addPromisesToClass(PromiseDependency) {
    this.prototype.resolvePromise = aws_util_1.default.promisifyMethod('resolve', PromiseDependency);
};
exports.ConfigProviderChain.deletePromisesFromClass = function deletePromisesFromClass() {
    delete this.prototype.resolvePromise;
};
aws_util_1.default.addPromises(exports.ConfigProviderChain);
...
exports.EnvironmentConfiguration = aws_util_1.default.inherit(rstreams_configuration_1.default, {
    ...
    refresh: function refresh(callback) {
        if (!callback)
            callback = aws_util_1.default.fn.callback;
        if (!process || !process.env) {
            callback(aws_util_1.default.error(new Error(`Unable to parse environment variable: ${this.envPrefix}.`), { code: 'EnvironmentConfigurationProviderFailure' }));
            return;
        }
        let values = null;
        if (process.env[this.envPrefix] != null) {
            try {
                ...
                callback(aws_util_1.default.error(new Error(`Unable to parse env variable: ${this.envPrefix}`), { code: 'EnvironmentConfigurationProviderFailure' }));
                return;
            }
        }
        else {
            ...
            for (let i = 0; i < keys.length; i++) {
                ...
                if (!values[key] && key !== 'LeoSettings') {
                    callback(aws_util_1.default.error(new Error('Variable ' + prefix + key + ' not set.'), { code: 'EnvironmentConfigurationProviderFailure' }));
                    return;
                }
            }
        }
...
exports.FileTreeConfiguration = aws_util_1.default.inherit(rstreams_configuration_1.default, {
    ...
    refresh: function refresh(callback) {
        ...
        if (values == null) {
            callback(aws_util_1.default.error(new Error(`Unable to find file config`), { code: 'FileTreeConfigurationProviderFailure', errors: errors }));
            return;
        }
        this.expired = false;
        rstreams_configuration_1.default.call(this, values);
...
exports.LeoConfiguration = aws_util_1.default.inherit(rstreams_configuration_1.default, {
    ...
    refresh: function refresh(callback) {
        ...
        if (values == null) {
            callback(aws_util_1.default.error(new Error(`Unable to get config from leo-config env ${config.env}`), { code: 'LeoConfigurationProviderFailure' }));
            return;
        }
        this.expired = false;
        rstreams_configuration_1.default.call(this, values);
...
exports.ObjectConfiguration = aws_util_1.default.inherit(rstreams_configuration_1.default, {
    ...
    refresh: function refresh(callback) {
        ...
        if (this.root == null || this.field == null || this.field == "") {
            callback(aws_util_1.default.error(new Error(`Root and Field must be specified.`), { code: 'ObjectConfigurationProviderFailure' }));
            return;
        }
        let values = this.root[this.field] ? this.root[this.field] : null;
        if (values == null) {
            callback(aws_util_1.default.error(new Error(`Unable to get config from ${this.field}`), { code: 'ObjectConfigurationProviderFailure' }));
            return;
        }
        this.expired = false;
        rstreams_configuration_1.default.call(this, values);
...
exports.AWSSecretsConfiguration = aws_util_1.default.inherit(rstreams_configuration_1.default, {
    ...
    refresh: async function refresh(callback) {
        ...
        if (!process || !process.env || !process.env[this.secretEnvKey]) {
            callback(aws_util_1.default.error(new Error(`Secret not specified.  Use ENV var ${this.secretEnvKey}.`), { code: 'AWSSecretsConfigurationProviderFailure' }));
            return;
        }
        let values = null;
        ...
        try {
            ...
            try {
                ...
                callback(aws_util_1.default.error(new Error(`Unable to parse secret '${secretKey}'.`), { code: 'AWSSecretsConfigurationProviderFailure' }));
                return;
            }
        }
        catch (err) {
            callback(aws_util_1.default.error(new Error(`Secret '${secretKey}' not found.`), { code: 'AWSSecretsConfigurationProviderFailure' }));
            return;
        }
        this.expired = false;
        rstreams_configuration_1.default.call(this, values);
