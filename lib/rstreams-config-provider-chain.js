...
exports.ConfigProviderChain = exports.ProvidersInputType = exports.AWSSecretsConfiguration = exports.ObjectConfiguration = exports.LeoConfiguration = exports.FileTreeConfiguration = exports.EnvironmentConfiguration = void 0;
    ...
    refreshSync() {
        ...
        outer: for (const dir of dirs) {
            for (const filename of this.filenames) {
                const file = path_1.default.resolve(dir, filename);
        ...
        if (values == null) {
            throw aws_util_1.default.error(new Error(`Unable to find file config`), { code: 'FileTreeConfigurationProviderFailure',
                errors: errors });
        }
        this.expired = false;
        this.update(values);
...
class LeoConfiguration extends rstreams_configuration_1.default {
    refreshSync() {
        ...
        const values = config.leosdk || config.leo_sdk || config["leo-sdk"] ||
...
class AWSSecretsConfiguration extends rstreams_configuration_1.default {
    ...
    refreshSync() {
        ...
        if (values == null) {
            let error = null;
            try {
                const value = new aws_sdk_sync_1.default.SecretsManager({
                    region: region
                ...
                error = aws_util_1.default.error(new Error(`Secret '${secretKey}' not available. ${err}`), { code: 'AWSSecretsConfigurationProviderFailure',
                    parent: err });
            }
            if (error != null) {
                throw error;
...
class ConfigProviderChain extends rstreams_configuration_1.default {
    ...
    resolveSync() {
        ...
        for (const provider of providers) {
            ...
            try {
                if (value instanceof rstreams_configuration_1.default) {
                    value.getSync();
                    return value;
                }
                return value;
...
ConfigProviderChain.defaultProviders = [
    ...
    function () {
        return new EnvironmentConfiguration('RSTREAMS_CONFIG');
    },
    /* Leo Env locations */
    function () {
        return new EnvironmentConfiguration('leosdk');
    },
    function () {
        return new EnvironmentConfiguration('leo-sdk');
    },
    function () {
        return new EnvironmentConfiguration('LEOSDK');
    },
    ...
    /* Object locations - process */
    function () {
        return new ObjectConfiguration(process, "leosdk");
    },
    function () {
        return new ObjectConfiguration(process, "leo-sdk");
    },
    function () {
        return new ObjectConfiguration(process, "rstreams_config");
    },
    /*  Object locations - global */
    function () {
        return new ObjectConfiguration(global, "leosdk");
    },
    function () {
        return new ObjectConfiguration(global, "leo-sdk");
    },
    function () {
        return new ObjectConfiguration(global, "rstreams_config");
    },
    /* File tree locations */
    function () {
        return new FileTreeConfiguration(process.cwd(), [
    ...
    function () {
        return new AWSSecretsConfiguration('LEO_CONFIG_SECRET');
    },
    function () {
        return new AWSSecretsConfiguration('RSTREAMS_CONFIG_SECRET');
    },
];
