...
Replace the ???? values in this example with the appropriate values from your installation.  
The values can be found in Leo console.  For more information on how to obtain these values, see [AWS Configuration](https://docs.leoplatform.io/docs/aws-configuration)


Issue the following command from your project directory, you will be prompted for the values:

```
$ node node_modules/leo-sdk/configure.js 
	Leo profile to configure? [Enter for default] 
	AWS Region? [Enter to skip] us-west-2
	Kinesis stream? [Enter to skip] Leo-KinesisStream-?????????
	S3 stream? [Enter to skip] leo-s3bus-?????????
	Firehose stream? [Enter to skip] Leo-BusToS3-?????????
	AWS Profile? [Enter to skip] default

Leo profile "default" updated!
```

This will create a file in your home directory `~/.leo/config.json` that contains your settings.  You can setup multiple profiles just like you can do with the AWS SDK.  

There are several ways to set the LEO profile and/or update the configuration at runtime. 

1. Set the Environment variable `LEO_DEFAULT_PROFILE`

This will cause the SDK to use the profile specified by this variable.  If no environment variable is set, it will attempt to load the profile called *default*.
If no profiles are configured on your system, you must pass in the full configuration when you require the sdk.

2. Set profile upon require
```
var leo = require("leo-sdk")("leo");
```

This will cause the SDK to use the profile called *leo*.

3. Call setProfile
```
var leo = require("leo-sdk");
leo.configuraion.setProfile("leo");
```

This accomplishes the same thing as setting the profile upon require.

4. Call configuration.update
```
var leo = require("leo-sdk");
leo.configuration.update("leo");
```

This is also the equivalent of the above methods.  

With options 2,3, and 4 above, the script will set the environment variable discussed in option 1 at runtime if it isn't set before. Subsequent runs will utilize this variable.

5. Override configuration values
```
var leo = require("leo-sdk")({
	s3: "leo-s3bus-????????",
...
This will load whatever profile is configured with the environment variable, or the default profile if nothing specified, or if no profiles are configured no values. 
It then will override the base config with the values passed in.
...
* **enableLogging:** if true, your process's log output will be sent to AWS and associated with this bot to view through the UI *default:* false
* **autoDetectPayload:** If this is false, then you will need pass a full event envelope along with your payload. If it is true, what you pass in will be the payload of the event.  *default:* true
...
//Create the leo object with the default leo profile
var leo = require("leo-sdk")
